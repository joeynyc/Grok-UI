...
## How this fits Grok UI

Grok UI's v0.11 secure remote sessions (Tailscale / SSH host-agent) **stay**.
That control plane is not `gbr/1`. Build Remote Agent is a **separate** pairing
device: store app + GitHub/HTTPS relay. Do not merge fleet tokens with mailbox
keys. Do not invent a fourth pair protocol.
